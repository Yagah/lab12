...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Invalid line format: " + line); //throw a TreeException with a useful message
		...
		else
		{
			parentNode = root.getNodeWithName(parent);  //There's a method in Node that searches for a named node. 
				if(parentNode == null) {
					throw new TreeException("Parent node not found: " + parent);//If the parent node wasn't found, there must have been something wrong in the 
				}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) {
			throw new TreeException("Node not found" + node1); //Throw a TreeException with a useful message
		}
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null) {
			throw new TreeException("Node not found" + node2);//Throw TreeException with a useful message
		}
